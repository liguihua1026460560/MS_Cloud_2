...
public class CopyObjectFileRebuilder {
    ...
    public Mono<Boolean> rebuildCopyObjFile(StoragePool pool, String metaKey, String lun, String errorIndex, String fileName, String endIndex, String fileSize, String crypto, String secretKey, List<Tuple3<String, String, String>> nodeList, String flushStamp, int retry) {
        return putCopyObjFileMeta(lun, fileName, metaKey)
                .flatMap(b -> {
                    ...
                    if (lockSuccess) {
                        ...
                        return Mono.delay(Duration.ofMillis(200L * (retry * 10L))).flatMap(wait -> rebuildCopyObjFile(pool, metaKey, lun, errorIndex, fileName, endIndex, fileSize, crypto, secretKey, nodeList, flushStamp, retry + 1));
                    }
                });
    }
