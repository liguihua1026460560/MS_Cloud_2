 ...
 * @author zhaoyang
 * @date 2026/02/05
 * 通过对byte[] 进行包装 实现零拷贝 ；只适用于 外部byte[]不会修改 且 buffer只读的情形
 **/
public class BytesWrapperBuffer implements Buffer {

    private ByteBuf buffer;

    public BytesWrapperBuffer() {
        this(0);
    }

    public BytesWrapperBuffer(int initialSizeHint) {
     ...
     * @param bytes
     */
    public BytesWrapperBuffer(byte[] bytes) {
//        buffer = Unpooled.unreleasableBuffer(Unpooled.buffer(bytes.length, Integer.MAX_VALUE)).writeBytes(bytes);
        buffer = Unpooled.unreleasableBuffer(Unpooled.wrappedBuffer(bytes));
    }

    public BytesWrapperBuffer(String str, String enc) {
        this(str.getBytes(Charset.forName(Objects.requireNonNull(enc))));
    }

    public BytesWrapperBuffer(String str, Charset cs) {
        this(str.getBytes(cs));
    }

    public BytesWrapperBuffer(String str) {
        this(str, StandardCharsets.UTF_8);
    }

    BytesWrapperBuffer(ByteBuf buffer) {
    ...
    public Buffer getBuffer(int start, int end) {
        return new BytesWrapperBuffer(getBytes(start, end));
    }
    ...
    public BytesWrapperBuffer setBytes(int pos, ByteBuffer b) {
    ...
    public Buffer copy() {
        return new BytesWrapperBuffer(buffer.copy());
    }

    public Buffer slice() {
        return new BytesWrapperBuffer(buffer.slice());
    }

    public Buffer slice(int start, int end) {
        return new BytesWrapperBuffer(buffer.slice(start, end - start));
    }
    ...
    public boolean equals(Object o) {
        ...
        BytesWrapperBuffer buffer1 = (BytesWrapperBuffer) o;
