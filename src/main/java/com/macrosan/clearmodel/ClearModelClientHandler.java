...
@Log4j2
public class ClearModelClientHandler {
    ...
    public ClearModelClientHandler(UnicastProcessor<SocketReqMsg> msgUnicastProcessor,
    ...
    public ClearModelClientHandler(StoragePool storagePool, UnicastProcessor<SocketReqMsg> msgUnicastProcessor,
        ...
        try {
            ...
            if (result.size() > 1000) {
                ...
                if (StringUtils.isNotEmpty(lifecycleRecord) && !restartSign.get() && beginPrefix.compareTo(lifecycleRecord) >= 0) {
                    ...
                    log.debug("bucket: {}, vnode: {}, list object complete!!!", bucketName, vnode);
                } else {
                ...
                if (restartSign.get()) {
                    ...
                    msg.put("beginPrefix", beginPrefix);
                    msgUnicastProcessor.onNext(msg);
                    log.debug("bucket: {}, vnode: {}, restart list object!!!", bucketName, vnode);
                } else {
                    msgUnicastProcessor.onComplete();
                    log.debug("bucket: {}, vnode: {}, list object complete!!!", bucketName, vnode);
                }
    ...
    private boolean checkSuccessNum() {
        if (responseInfo.successNum == 0 && (storagePool.getK() + storagePool.getM()) > 1) {
            log.error("Clear model get object list error!");
            msgUnicastProcessor.onComplete();
            return true;
            ...
            if (retryTimes > 10) {
                ...
                log.info("Clear model get object list error, retry " + retryTimes + "th !");
                msgUnicastProcessor.onNext(msg);
            }
            return true;
        }
