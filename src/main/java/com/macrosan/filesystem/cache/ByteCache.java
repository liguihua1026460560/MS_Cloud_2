...
public class ByteCache {
    ...
    private ByteCache(long offset, int size, byte[] data) {
        this.offset = offset;
        this.size = size;
        this.data = data;
    }

    public static ByteCache alloc(long offset, int size) {
        byte[] data = BytesPool.allocate();
        if(data == null) {
            return null;
        }

        return new ByteCache(offset, size, data);
    }
