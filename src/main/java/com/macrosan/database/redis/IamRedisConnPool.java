 ...
 * @date 2018/11/20
 */
public class IamRedisConnPool {
    private static final Logger logger = LogManager.getLogger(IamRedisConnPool.class.getName());

    private static IamRedisConnPool instance;
    ...
    public static IamRedisConnPool getInstance() {
        return instance;
    }
    ...
    public static void init() {
        if (instance == null) {
            instance = new IamRedisConnPool();
        }
    }

    private IamRedisConnPool() {
    ...
    private RedisURI getUri(String host, int database) {
        return RedisURI.Builder.redis(host, 6381)
                .withPassword(REDIS_PASSWD)
                .withDatabase(database)
                .build();
    }
    ...
    private FastList<RedisURI> getNodes(int database) {
        return new FastList<RedisURI>(4)
                .with(getSocketUri(UDS_6381_ADDRESS, database), getUri(masterVip1, database), getUri(masterVip2, database));    }
