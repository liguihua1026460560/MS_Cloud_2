...
@Log4j2
public class ShardingRightExpansionTaskRunner extends AbstractShardingTaskRunner {
    public ShardingRightExpansionTaskRunner(String bucketName, String sourceVnode, String targetVnode, long copyMaxKeys) {
        super(bucketName, sourceVnode, targetVnode, copyMaxKeys, AbstractCopyTaskRunner.Direction.RIGHT, Type.REXPANSION);
        ...
        if (src.parent != null) {
            if (isLeft(src)) {
                ...
                ObjectSplitTree.Node rightNeighbor = objectSplitTree.rightNeighbor(src);
                if (rightNeighbor != null) {
                    ObjectSplitTree.Node intersection = ObjectSplitTree.intersection(rightNeighbor, src);
    ...
    protected Mono<Boolean> remove() {
        ...
        if (discard.get()) {
            ...
            return realRemove(targetVnode, AbstractRemoveTaskRunner.POSITION.RIGHT);
        }
        return removeRepeat();
    }
        ...
        Tuple2<String, AbstractRemoveTaskRunner.POSITION> task1 = new Tuple2<>(targetVnode, AbstractRemoveTaskRunner.POSITION.RIGHT);
        Tuple2<String, AbstractRemoveTaskRunner.POSITION> task2 = new Tuple2<>(sourceVnode, AbstractRemoveTaskRunner.POSITION.LEFT);
